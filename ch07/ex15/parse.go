...
func Parse(input string) (_ Expr, err error) {
	return parseAndInquireVar(input, nil)
}

func parseAndInquireVar(input string, env Env) (_ Expr, err error) {
	defer func() {
...
func parseExpr(lex *lexer, env Env) Expr { return parseBinary(lex, 1, env) }

// binary = unary ('+' binary)*
// parseBinary stops when it encounters an
// operator of lower precedence than prec1.
func parseBinary(lex *lexer, prec1 int, env Env) Expr {
	lhs := parseUnary(lex, env)
	for prec := precedence(lex.token); prec >= prec1; prec-- {
		for precedence(lex.token) == prec {
			op := lex.token
			lex.next() // consume operator
			rhs := parseBinary(lex, prec+1, env)
...
// unary = '+' expr | primary
func parseUnary(lex *lexer, env Env) Expr {
	if lex.token == '+' || lex.token == '-' {
		op := lex.token
		lex.next() // consume '+' or '-'
		return unary{op, parseUnary(lex, env)}
	}
	return parsePrimary(lex, env)
}
...
//         | num
//         | '(' expr ')'
func parsePrimary(lex *lexer, env Env) Expr {
	switch lex.token {
	case scanner.Ident:
		id := lex.text()
		...
		if lex.token != '(' {
			if env != nil {
				...
				if !ok {
					...
					if err != nil {
						panic(fmt.Sprintf("parsing value to set variable: %v", err))
					}
					env[Var(id)] = value
				}
			}
		...
		if lex.token != ')' {
			for {
				args = append(args, parseExpr(lex, env))
				if lex.token != ',' {
					break
				}
				lex.next() // consume ','
